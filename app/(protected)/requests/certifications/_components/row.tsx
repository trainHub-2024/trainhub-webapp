...
const RequestStatusDisplay = ({
  ...
  return (
    <span
      className={`capitalize text-sm border px-3 py-1 rounded-full ${style[status]}`}
    >
      {status}
    </span>
  );
  ...
  async function handleChangeStatus(newStatus: "denied" | "completed") {
    ...
    try {
      const res = await updateAdminRequestStatus({
        id: data.$id,
        status: newStatus,
        trainer_id: data.trainerProfile_id,
        type: data.type,
      });
  ...
  return (
    <TableRow>
      <TableCell className="capitalize">
        {data?.trainerProfile?.name} {data?.userProfile?.name}
      </TableCell>
      <TableCell className="font-medium text-center">
        <RequestStatusDisplay status={data.status} />
      </TableCell>
      <TableCell className="text-center">{data?.text}</TableCell>
      <TableCell className="uppercase text-center">{data.type}</TableCell>
      <TableCell>
        {data?.certification && (
