...
export function PaidOverview({ earnings }: { earnings: Appointment[] }) {
    const totalBalance = earnings.reduce((sum, account) => sum + account.price, 0)
                ...
                <div className="space-y-2">
                    {earnings.map((a) => (
                        <div key={a.$id} className="flex justify-between items-center">
                            <span className="text-sm text-muted-foreground capitalize">{a.userProfile.name} - {new Date(a.date).toLocaleString()}</span>
